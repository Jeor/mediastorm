...
func TestHomeDesignerAssetsServeOnlyEmbeddedAllowlistEntries(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "canvas module", asset: "canvas.js", want: http.StatusOK, wantContentType: "text/javascript; charset=utf-8"},
		{name: "retired outline module", asset: "outline.js", want: http.StatusNotFound},
