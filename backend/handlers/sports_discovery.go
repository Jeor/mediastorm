...
func (h *LiveHandler) fetchSportsAddon(ctx context.Context, manifestURL, proxy string, d *sportsDiscoveryRequest) ([]LiveChannel, error) {
	...
	if query != "" && !d.broad {
		for _, c := range candidates {
			...
			if len(selectableSportsMatches(matchGameToChannels(d.game, filterSportsChannelsByScope(channels, nil, d.categories), nil, ""))) > 0 {
				return channels, nil
			}
			break
		}
