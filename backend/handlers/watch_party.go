...
func (h *WatchPartyHandler) CreateInvitation(w http.ResponseWriter, r *http.Request) {
	...
	writeWatchRoomJSON(w, http.StatusCreated, watchPartyInviteView{
		URL: h.serverBasePath + "/watch-party/" + invite.Token, LandingURL: h.serverBasePath + "/watch-party",
...
func (h *WatchPartyHandler) Landing(w http.ResponseWriter, _ *http.Request) {
	h.renderPage(w, "Join a Watch Together room", `<p>Enter the code shown on the host's TV.</p>
		<form method="post" action="`+html.EscapeString(h.serverBasePath)+`/watch-party/code"><input name="code" maxlength="9" autocomplete="one-time-code" placeholder="ABCD-EFGH" required><button type="submit">Continue</button></form>`)
}

func (h *WatchPartyHandler) ResolveCode(w http.ResponseWriter, r *http.Request) {
	code := strings.ToUpper(strings.TrimSpace(r.FormValue("code")))
	if _, err := h.service.ResolveExternalInvitation(r.Context(), code, true); err != nil {
		h.renderUnavailable(w)
		return
	}
	http.Redirect(w, r, h.serverBasePath+"/watch-party/code/"+code, http.StatusSeeOther)
}
...
func externalRoomView(room *models.WatchRoom) watchPartyRoomView {
...
func (h *WatchPartyHandler) RoomPage(w http.ResponseWriter, r *http.Request) {
	...
	content := `<p>Connected to <strong>` + html.EscapeString(room.Title) + `</strong>.</p><div id="status">Waiting for the host…</div><div id="members"></div>
	<script>const roomId=` + string(mustJSON(roomID)) + `;let grantRequested=false;async function refresh(){try{const base=` + string(mustJSON(h.serverBasePath)) + `;const r=await fetch(base+'/api/watch-party/room/'+encodeURIComponent(roomId),{credentials:'same-origin'});if(!r.ok)throw new Error();const room=await r.json();document.getElementById('status').textContent=room.status==='lobby'?'Waiting for the host to start…':room.status==='ended'?'This room has ended.':room.playbackReady?'Joining playback…':'The host is preparing playback…';document.getElementById('members').innerHTML='<h2>In the room</h2>'+room.members.map(m=>'<div class="member">'+escapeHTML(m.name)+(m.isCreator?' · Host':'')+'</div>').join('');if(room.status==='playing'&&room.playbackReady&&!grantRequested){grantRequested=true;const grant=await fetch(base+'/api/watch-party/room/'+encodeURIComponent(roomId)+'/playback-grant',{method:'POST',credentials:'same-origin'});if(!grant.ok)throw new Error();const payload=await grant.json();window.location.replace(payload.url);}}catch{grantRequested=false;document.getElementById('status').textContent='Unable to reach the room.'}}function escapeHTML(v){const d=document.createElement('div');d.textContent=v||'';return d.innerHTML}refresh();setInterval(refresh,2000);</script>`
