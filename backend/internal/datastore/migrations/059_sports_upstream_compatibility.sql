-- +goose Up
-- Early Sports Hub branches used versions 56–58 for sports tables. Goose only
-- records numbers, so those databases otherwise skip upstream's migrations at
-- the same versions. Restore the missing upstream schema without changing its
-- migration history or modifying installations that already applied it.
-- +goose StatementBegin
DO $$
BEGIN
    IF to_regclass('remote_access_pairings') IS NULL THEN
...
ALTER TABLE notification_channels ADD COLUMN IF NOT EXISTS include_profile_name BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE notification_channels ADD COLUMN IF NOT EXISTS include_device_name BOOLEAN NOT NULL DEFAULT FALSE;

-- +goose Down
-- Compatibility repair only: preserve upstream tables and existing pairing data.
SELECT 1;
