...
func (d *Deobfuscator) DeobfuscateFilename(filename string, allFiles []nzbparser.NzbFile, currentFile nzbparser.NzbFile) *DeobfuscateResult {
	...
	if par2Name := d.extractFromPar2Files(allFiles, filename); par2Name != "" {
		// Clean up PAR2 volume indicators from the filename
		cleanedName := d.removePar2VolumeIndicators(par2Name)
		result.DeobfuscatedFilename = cleanedName
...
// removePar2VolumeIndicators removes PAR2 volume patterns from filenames
// Patterns like: .vol-01, .vol01, .vol001 etc. before the extension
func (d *Deobfuscator) removePar2VolumeIndicators(filename string) string {
	// Get extension
	ext := filepath.Ext(filename)
	if ext == "" {
		return filename
	...
	// Common PAR2 volume patterns to remove:
	// - .vol-01, .vol-001, .vol-0001
	...
	for _, pattern := range patterns {
		...
		if re.MatchString(cleaned) {
			...
			d.log.Debug("Removed PAR2 volume indicator",
				"original", baseName,
				"cleaned", cleaned,
				"pattern", pattern)
			break
