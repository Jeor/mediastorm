...
func (p *Parser) fetchYencHeaders(ctx context.Context, segment nzbparser.NzbSegment, groups []string) (nntpcli.YencHeaders, error) {
	...
	err = retry.Do(
		func() error {
			...
			// A sibling file parser may cancel this context while BodyReader is
			// returning. Some nntppool versions can then return a non-nil wrapper
			// whose internal reader is nil, so never call into it after cancellation.
			h, err := getYencHeadersWithContext(ctx, r)
			if err != nil {
