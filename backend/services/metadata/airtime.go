...
func applyAirTimeFromTVDB(title *models.Title, airsTime string, networkName string, networkCountry string) {
	...
	if title.AirsTimezone == "" && title.AirsTime != "" {
		tz := ""
		// TVDB records Netflix as a US network even when airsTime is the local
		// release time for an international original. Prefer the series' origin
		// country in that case so, for example, a 16:00 Korean release is not
		// interpreted as 16:00 Pacific time.
		originCountry := strings.ToLower(strings.TrimSpace(title.CountryCode))
		if strings.Contains(strings.ToLower(networkName), "netflix") && originCountry != "" && originCountry != "usa" {
			tz = inferTimezoneFromNetwork("", title.CountryCode)
		}
		if tz == "" {
